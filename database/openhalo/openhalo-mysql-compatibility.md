...
> **30 秒答案**：OpenHalo 在 PG 14.18 内核的 9 个关键点（监听→认证→引擎→读命令→COM→解析→规划→执行→回包）插入 MySQL 协议分支，通过双端口（5432/3306）+ 双协议虚表实现一套代码同时服务 PG 和 MySQL 客户端。主链改动集中在 F1–F9 九处分叉，共 619 个 diff 项。

> **文档版本**：2026-07-17（OpenHalo 研究报告；aux_mysql 对象规模按源码严格口径复核） 
...
本文档只描述 OpenHalo 相对 PG14.18 的增量；源码路径均相对于 `openHalo/` 仓库根。若本地没有对照树，仍可按 §3.1 的「动作 + 证据位置」建立主链心智模型，行号以文中标注为准。

先读 §3.1 时间线，再按 F1→F9 走 §5–§13；§4 为分叉与虚表速查。

阅读路径（按目标选，不必通读）：

- 快速（约 20 分钟）：§1 → §3.1 → §17 → §19.3。
- 主链（约 2 小时）：§2 → §3 → §4 → §5–§13。
- 检索（按需）：§20（先 §20.2 或按路径搜索；功能看 A+B，跳过 §20.5 D）→ §21 模块入口 → §22 协议/报文 → §23 adt·aux_mysql·符号表。行级 diff：`diff -u postgresql-14.18/<path> openHalo/<path>`。

### 1.2 核心术语（首次阅读 inline 定义）

- **协议虚表**（`ProtocolInterface` / `ProtocolRoutine`）：**20** 个函数指针 + `type` 字段（含 4 个 `printtup*` 分槽），从 listen 到回包贯穿连接生命周期；详见 §4.2。
...
- **COM**（Command）：MySQL 包 payload 首字节（0–31，含 `SLEEP`=0）；F4 `readCommand` 返回，F5 `processCommand` 消费（§8–§9）。
...
- **`netTransceiver`**：MySQL **4 字节包头**（3 字节长度 + 1 字节 seq）+ payload 读写；F2 初始化，F4/F9 I/O（§6）。
- **`process_command` 返回值**：`1` = adapter 已回包、主循环 `continue`（**不进 F6**）；`0` = 继续 `exec_simple_query`（§9、§3.2）。
- **`aux_mysql`**：`contrib/aux_mysql` 扩展，**1828** 条 `CREATE FUNCTION`（`mysql.*` **1823**；行首严格匹配 `^create (or replace )?function`，见 §19.2 / §23.2）；须 `CREATE EXTENSION`（§14）。
...
PostgreSQL 14 处理一条客户端 SQL 的**时间顺序**如下。OpenHalo 在标有 **★（F1–F9）** 的环节插入分叉，其余大多复用 PG 存储、事务、锁、WAL。

```mermaid
flowchart TB
    C[客户端] --> PM[PostmasterMain]
    PM --> L["★ F1 StreamServerPort<br/>填 ListenSocket"]
    L --> SL[ServerLoop]
    SL --> CC["★ F1 ConnCreate<br/>accept"]
    CC --> BS[BackendStartup fork]
    BS --> BI[BackendInitialize]
    BI --> PSP["★ F2 init/start<br/>替代 ProcessStartupPacket"]
    PSP --> IP["★ F2 InitPostgres<br/>authenticate"]
    IP --> ENG["★ F3 InitParserEngine<br/>InitExecutorEngine"]
    ENG --> PMN[PostgresMain]
    PMN --> RC["★ F4 ReadCommand"]
    RC --> PC["★ F5 process_command<br/>COM 预处理"]
    PC --> ESQ[exec_simple_query]
    ESQ --> RP["★ F6 raw_parser"]
    RP --> PA["★ F7 parse_analyze<br/>transformStmt"]
    PA --> PL["★ F7 planner"]
    PL --> EX["★ F8 ExecutorStart/Run"]
    EX --> PT["★ F9 printtup / end_command<br/>客户端"]
```
...
OpenHalo 仅在标 **★** 的节点注入 MySQL 路径；存储、事务、锁、WAL 等大多复用 PG。MySQL 连接**不替代** `PostgresMain`。`mainFunc` 仍调用同一主循环（`adapter.c:805–807`），差异经协议虚表与 `*Routine` 注入。**§3.1** 用 `SELECT 1` 串起全流程；§5–§13 逐点展开；§4 为分叉总述与虚表速查。
...
| 典型用途 | 业务 SQL | 连接保活探测 |
| 判据记忆 | COM 需要桥接为 PG `'Q'` 才进 `exec_simple_query` | COM 在 adapter 内闭环，无需 parser |
...
`ProtocolInterface` 定义于 `src/include/postmaster/protocol_interface.h`（typedef 名 `ProtocolRoutine`，下文统称 **协议虚表**；含 `type` + **20** 个函数指针，其中 `printtup` / `printtup_startup` / `printtup_shutdown` / `printtup_destroy` 各占一槽）。OpenHalo 用两套静态实例：**PG 标准** `standard_protocol_handler`（`postmaster2.c:52`）与 **MySQL** `protocolHandler`（`adapter.c:464`）。下表为速查，每槽位附 5–10 字用途；**时间顺序与逐步 diff 以 §5–§13 为准**，此处不重复展开。
...
> §5–§13 各节细节展开见正文；**619 项 diff 按路径/目录聚合索引**见 [§20](#20-全量-diff-对照索引)（勿逐节重复跳转）。
...
**MySQL fd 注册**：`initListen`（`adapter.c:517–637`）复用 PG 的 `listen_addresses` GUC，对每个地址调 `createServerPort(..., &protocolHandler)`（`:3700`），成功 bind 后 `setSecondProtocolSocket(fd)`（`:3919`）。这等价于 `ListenHandler[i] = protocolHandler`（`T_MySQLProtocol`，`:464–486`），端口取自 GUC `mysql.port`。
...
flowchart TB
    PM[PostmasterMain]
    PM --> SSP[StreamServerPort PG 端口]
    SSP --> SSS["setStandardProtocolSocket<br/>ListenHandler=standard"]
    PM --> GUC{"database_compat_mode=mysql<br/>listener_on=on"}
    GUC -->|是| LI["secondProtocolHandler<br/>listen_init"]
    LI --> ML["MySQL 端口 bind<br/>ListenHandler=protocolHandler"]
    GUC -->|否| SL[ServerLoop]
    SSS --> SL
    ML --> SL
    SL --> ACC[ConnCreate serverFd]
    ACC --> GPH[getProtocolHandlerByFd]
    GPH --> AH["protocol_handler<br/>accept"]
    AH --> FORK["fork<br/>BackendInitialize"]
```
...
**连接初始化**：PG14 直接 `pq_init()`；OpenHalo 改为 `port->protocol_handler->init()`（`postmaster.c:4418`）。PG 路径走 `standard_init` → `pq_init`（`postmaster2.c:245–247`）；MySQL 路径 `initServer`（`adapter.c:652–678`）调 `initNetTransceiver()`、建 `FeBeWaitSet`、设 nonblocking socket，**不走 libpq 消息帧**。
...
**主入口与错误回包**：PG14 `BackendRun` 末尾直接 `PostgresMain`；OpenHalo（`:4577–4580`）优先 `protocol_handler->mainfunc`。MySQL 的 `mainFunc`（`:805–807`）内部仍调 `PostgresMain`，主循环不变。错误路径须同样走 MySQL Error 包，故 `elog.c` 改为虚表分发：
...
F4 对应 §3.1 步骤 7 与 §3.2 步骤 1：`PostgresMain` 主循环每轮先 `ReadCommand` 读入一条客户端消息。**输入**为 F3 就绪的引擎指针与 F2 初始化的 `netTransceiver`；**输出**为 `firstchar`（MySQL 为 COM 字节 0–31；PG 为 `'Q'`/`'P'` 等 ASCII message type）+ `input_message`（含 SQL 正文）。**COM** = MySQL payload 首字节，供 F5 `processCommand` 消费；`StringInfo.offset=128` 预留包头空间（§1.2）。
...
```mermaid
flowchart TB
    RC["readCommand<br/>返回 COM 字节"] --> PC[processCommand]
    PC -->|return 1| RFQ["send_ready_for_query<br/>continue"]
    PC -->|"return 0<br/>Q"| ESQ[exec_simple_query]
    ESQ --> SESQ["standard_exec<br/>simple_query"]
...
PG14 `parse_analyze` → `transformStmt`（`analyze.c` 内直接实现）将 raw tree 转为 `Query`；`planner()` 生成 `PlannedStmt`，无引擎虚表，也无 MySQL 命名空间映射。
...
**Executor 虚表**：PG14 `ExecutorStart`/`ExecutorRun` 无 hook；MySQL 虚表 `mys_executor_engine`（`mys_executor.c:30–40`）提供 `mys_ExecutorStart`、`mys_ExecutorRun`、`mys_ExecInitNode` 等，内部包装标准 Executor 并注入 MySQL DML 语义，如 `INSERT IGNORE`、`ON DUPLICATE KEY` 等于 `mys_nodeModifyTable.c`（3958 行）；MySQL 分区路由于 `mys_execPartition.c`。
...
一轮命令完成；主循环回到 F4 `ReadCommand` 等待下一条 COM/SQL。§14–§18 讲并行支撑层（函数/GUC/AST/地图/限制）：不占 F1–F9 主时间线，但约束 §3 worked example 的边界。
...
**`contrib/aux_mysql/`**（纯新增）：control + SQL 迁移 + `mysql.conf`，安装后创建 `mysql`、`mys_sys` 等 schema，注册 `tinyint`、`datetime` 等 domain。共 **1828** 条 `CREATE FUNCTION`（`mysql.*` **1823**；行首严格匹配口径，见 §1.2 / §19.2），例如 `mysql.concat(text,text)` 在 SQL 层注册、C 实现在 `strfuncs.c`。**非 initdb 自动**，须 `CREATE EXTENSION aux_mysql;`。
...
本节是 §5–§13 主链的**路径级导读**：读 §3 时若见某 `文件:行`，可在此反查该文件还在哪些 F 阶段被触及。完整 619 项逐行摘要见 **§20**；此处只保留高频目录与统计，避免大表堆数字后再解释。
...
**大文件维护成本**：`mys_gram.y`、`mys_tablecmds.c` 逐规则覆盖，按需要检索而非通读。
...
**全量路径索引**（§20）：**619/619** 项，`diff -rq` 输出按**路径/目录聚合**索引（组标题 `#### \`src/backend/...\``）；B 类 367 项有 §20.2 导读，深度仍见 §5–§13 或按需 `diff -u`。
...
**② 行级核对（F1 双监听示例）**。已定位到具体文件、需要看改了哪些行时使用：
...
**③ aux_mysql 对象条数**。验证 §14 / §23.2 所述严格口径（期望：FUNCTION **1828** / `mysql.*` **1823**；DOMAIN **23**；TYPE **71**；CAST **57**）时使用：

```bash
grep -hiE '^create (or replace )?function' openHalo/contrib/aux_mysql/*.sql | wc -l
grep -hiE '^create (or replace )?function' openHalo/contrib/aux_mysql/*.sql \
 | grep -i 'mysql\.' | wc -l
grep -hiE '^create domain' openHalo/contrib/aux_mysql/*.sql | wc -l
grep -hiE '^create type' openHalo/contrib/aux_mysql/*.sql | wc -l
grep -hiE '^create cast' openHalo/contrib/aux_mysql/*.sql | wc -l
```
...
```mermaid
flowchart TB
    ssp["F1 StreamServerPort<br/>setStandardProtocolSocket"]
    li["F1 listen_init<br/>createServerPort<br/>setSecondProtocolSocket"]
    cc["F1 ConnCreate"]
    gph["F1 getProtocolHandlerByFd<br/>Port.protocol_handler"]
    acc["F1 protocol_handler<br/>accept"]
    init["F2 init<br/>initServer / netTransceiver"]
    start["F2 start<br/>startServer"]
    auth["F2 authenticate<br/>握手 + mysCheckAuth"]
    main["F2 mainFunc<br/>PostgresMain"]
    ie["F3 InitParserEngine<br/>InitExecutorEngine<br/>InitPlannerEngine"]
    pe["F3 parserengine / executorengine<br/>全局指针"]
    f4["F4 read_command<br/>COM 或 PG msgtype"]
    f5["F5 process_command<br/>短路或 HALO_REQ_QUERY"]
    esq["exec_simple_query<br/>standard_exec_simple_query"]
    ping["return 1<br/>OK 包 · continue"]
    f6["F6 parserengine<br/>raw_parser"]
    f7["F7 transformStmt<br/>+ 标准 planner"]
    f8["F8 executorengine<br/>+ ProcessUtility_hook"]
    f9["F9 printtup / end_command<br/>sendOKPacket/EOF"]
    ssp --> li
    li --> cc
    cc --> gph
    gph --> acc
...
说明：`return 1` 路径（PING 等）从 `ping` 回到 `f4`；`return 0 · Q` 进入 SQL 路径，F9 完成后同样回到 `f4`。F2 的 `send_message` 虚表在 ERROR 路径旁路发 MySQL Error 包，未单独画支路。
...
**如何读 §20 全索引（619 项）**：§20.1–§20.5 合计 619 条索引行，与 `diff -rq` 逐项对应，正文按**路径/目录聚合**呈现（非 619 段 inline 展开）。每条含「路径 + 变更类型 + 一句话摘要」；**不要从头通读**，用 §1.1 检索策略或 §20.2 按 F 分组缩小范围。主链约 40 个核心文件已在 §5–§13 展开；§20 补全其余项「是否存在、一句话用途」。只关心 MySQL 功能时跳过 **§20.5 类别 D**（207 项噪声）。
...
1. **按分叉点查主链文件**（最常用）。已知 F 编号、要找对应 diff 路径时使用：
...
| `src/include/postmaster/protocol_interface.h` | new 111L | ProtocolInterface：type + 20 函数指针虚表 |
...
协议适配层实现 **MySQL 线协议**与 PG 主循环的桥接：`protocolHandler` 虚表覆盖 F1–F5、F9；`netTransceiver` 负责 **4 字节包头**（3 字节长度 + 1 字节 seq）+ payload 读写；`userLogonAuth.c` / `pwdEncryptDecrypt.c` 负责 F2 认证；`errorConvertor.c` 将 PG SQLSTATE 映射为 MySQL 错误码。

#### 21.1.2 入口符号与文件

整个 `src/backend/adapter/` 为 **Only in openHalo**（7×`.c` + Makefile），`adapter.c` 约 6554 行。核心符号：**`protocolHandler`**（`adapter.c:464`，20 槽位虚表，`T_MySQLProtocol`）；F1 **`initListen` / `acceptConn`**（`:517` / `:640`）；F2 **`authenticate`**（`:690`）；F4/F5 **`readCommand` / `processCommand`**（`:852` / `:1267`）；F9 **`printTup*` / `endCommand`**（`:1086` / `:871`）；**`mainFunc`**（`:805`，包装 `PostgresMain` 共享主循环）。
...
扩展安装脚本：注册 **1828** 条 `CREATE FUNCTION`（`mysql.*` **1823**）、**23** 个 `CREATE DOMAIN` 等（严格口径见 §23.2）；SQL 层声明，多数 `LANGUAGE C` 指向 §21.5 的 C 实现。
...
*§20 索引 + §21 走读 = 全量 619 项 diff 的完整覆盖；§5–§13 仍为主链深度阅读入口。读完 §21.1 的入口符号后，若需 postmaster 内 **逐改动块** 与报文格式，继续 §22；§23 按需要检索 adt/aux_mysql 等补充材料。*
...
PG14 原版只有 `StreamConnection → ProcessStartupPacket → PostgresMain` 一条硬编码路径。OpenHalo **不替换** 这条路径，而是在 postmaster 侧增加第二监听与按 fd 选 handler，在 adapter 侧实现 MySQL 线协议；两层通过 **20 槽位 `ProtocolInterface` 虚表**（§4.2）接通。
...
执行器每产出一行 `TupleTableSlot`，**`DestReceiver.receiveSlot`** 将其转成线上报文：PG 路径发 libpq DataRow；MySQL 路径发 Field/Resultset/EOF。
...
**`aux_mysql`** 是纯新增扩展（control + 迁移 SQL + `mysql.conf`），版本链 **1.1 → 1.5**，迁移脚本合计约 **35k 行**。对象规模（行首严格匹配，与 §14 / §19.2 同一口径）：**1828** 条 `CREATE FUNCTION`（`mysql.*` **1823**）、**23** 个 `CREATE DOMAIN`、**71** 个 `CREATE TYPE`、**57** 个 `CREATE CAST`。
...
完整函数名枚举：`grep -hiE '^create (or replace )?function' openHalo/contrib/aux_mysql/*.sql | grep -i 'mysql\.' | sort -u`
...
- **`getMysProtocolHandler`**（`adapter.c:490`）— MySQL handler 工厂，F1
- **`getProtocolHandlerByFd`**（`postmaster2.c:153`）— socket → handler，F1 ConnCreate
- **`InitParserEngine`**（`parsereng.c:38`）— F3 解析引擎
- **`mys_raw_parser`**（`mys_parser.c:319`）— F6 入口
...
```mermaid
flowchart TD
    BM["backend/Makefile<br/>SUBDIRS"] --> AD[adapter/mysql]
    BM --> PR["parser/<br/>parsereng + parser/mysql"]
    BM --> EX["executor/<br/>mys_* + engine"]
    BM --> CM[commands/mysql]
    BM --> UT["utils/ddsm/mysm"]
    BM --> CT["catalog/<br/>namespace2.o"]
```
