...
`jointree` 注释写明它包含 FROM 与 WHERE（`parsenodes.h:185-186`）。Parser/Analyzer 节点对照见 [§1.5](#15-解析-vs-分析最易混)。

### RawStmt → Query：一句话对照

Parser 只认**语法槽位 + 名字**；Analyzer **查 catalog**，绑 OID/列号，吐出 **`Query`**。相当于把文法分好的 SQL **关联到本库实际对象**，**而不是**决定调用哪些执行函数（那是 **规划器** 产出的 **Plan** 层）。
