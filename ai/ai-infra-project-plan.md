...
> 生成日期：2026-06-17 | 状态：计划稿 | 最后更新：2026-07-17（v9.5：与 engineering 时效核查对齐，不改学习主线）  
> **当前进度未勾选，按 Step 顺序执行。**
...
本计划写的是可交付的 AI 工程落地路径，不是泛化的「AI Infra 学习大纲」。只照着跑 AI 生成的脚本，很难形成可复述、可排障的能力；系统工程经验更适合用来收敛问题、验证链路和推动交付。每个阶段按「问题单 → 小闭环 → 可讲产物」推进，再扩大功能。

执行原则：

1. 代码可以 AI 辅助生成，但核心路径必须能自己讲清。
2. Step 0–1 中每个核心脚本配一段 `notes/ownership.md`（解决什么问题、核心 API、如何验证）；Step 2 及以后在各项目 README 记录 AI 辅助边界与验证方式。
3. 每次只做一个小闭环，不以「完成整个项目」为日目标。
4. 每完成一个小闭环，至少留下一项可验证产物：机制说明、排障记录或一组 benchmark 数字。
5. 每个 Step 以真实问题开头，例如「接口为什么慢」「RAG 为什么答不准」「流式输出为什么不能直接 async generate」。
...
|------|------|------|------|
| 模型加载与基础推理 | HuggingFace Transformers、PyTorch | Transformers GitHub 约 17 万 stars（2026.7），PyPI 月下载量过亿，开源模型接入常用基础库 | 是，Step 1 |
| LLM 应用编排 | LangChain、LlamaIndex、LangGraph | LangChain 约 10 万 stars，生态大；LlamaIndex 在 RAG 场景增长快 | 部分覆盖，Step 3 不重依赖框架 |
| 高性能推理服务 | vLLM、SGLang、llama.cpp、Ollama | vLLM ~86k stars（查询 2026-07-17），主打 PagedAttention、Continuous Batching、OpenAI 兼容服务；Ollama stars 更高，更常用于本地开发体验 | 作为 P2 概念加分，Step 4 |
...
本计划选择「HF Transformers → LLM 流式服务化 → RAG → vLLM 概念」，覆盖模型接入、服务化、企业 RAG 与推理引擎概念，同时绕开 CUDA/C++ kernel 与大规模训练等高成本板块。

**证据来源：**
- HuggingFace Transformers：GitHub 约 17 万 stars（2026.7），PyPI 月下载量过亿，它是开源模型加载和推理的主流基础库。
- SLM：HF Hub 上 92.48% 模型下载为 1B 以下（下载口径，含 embedding 等）；按任务混合路由是常见做法，非统一生产标准。本计划选用 0.6B-1B 小模型，主因是学习成本可控。
- LangChain / LlamaIndex：LangChain 约 10 万 stars，LlamaIndex 在 RAG 场景持续增长。
- vLLM/SGLang：vLLM ~86k stars，SGLang ~30k stars；二者均可作生产 Serving。部分团队将其作为架构选项（vLLM 作基座、SGLang 覆盖部分加速/结构化输出），非唯一标准。
- 生产级 AI Stack 文章普遍把模型服务、RAG/Memory、Observability/Eval、部署层拆成独立模块。

### 1.4 为什么先做第一个技术 Step（Step 1）

Step 0 先确认代码 ownership；第一个技术 Step 选 HuggingFace Transformers，因为它是后面所有项目的共同地基。
...
| 可选方向 | 为什么不作为第一步 |
|------|------|
| 直接用 OpenAI / Claude API | **求职作品集**不够：练不到本地加载、tokenizer、量化与自托管指标。**做产品验证**时可先用商业 API：先证明用户愿意用，再把热点路径换成 SLM/自托管压成本 |
...
数月内完成一个可讲的 LLM 服务化项目，简历上形成第一条叙事线：**「C/C++ 系统工程背景 → HF Transformers 推理机制理解 → OpenAI 兼容流式服务 → TTFT/TPOT 可观测性 → Docker 交付」**。
...
5. **Step 4：vLLM 概念学习** — 面试概念加分，源码精读有余力再做。

**不做单一岗位锁定**。当前阶段的策略是：用项目产出覆盖常见技能交集，然后根据实际投递反馈调整方向。以下是技能组合与岗位的对应关系（基于 2026-06 深圳市场数据）：

| 岗位方向 | 匹配度 | 核心匹配项 | 需补的差距 |
|----------|:---:|------|------|
| **AI 平台后端开发** | 高 | FastAPI + Docker/K8s + Prometheus + HF 基础 | Step 0-1-2 先覆盖入场门槛，RAG/vLLM 作为后续加分 |
| **大模型应用开发** | 高 | HF + RAG + FastAPI + 服务化 | LangChain 实操（Step 3 已安排 LangChain 对比任务）、Agent 概念（了解但不实操） |
| **推理部署工程师** | 低 | vLLM + Docker/K8s + TTFT/TPOT | 缺少 GPU 多卡部署经验、CUDA 概念；暂不作为主线 |
| **ML Platform** | 中 | K8s + CI/CD + Prometheus + 模型服务化 | 缺少大规模集群经验、GPU 调度 |

> 本计划产出的技能组合与「AI 平台后端 / LLM 服务化 / 大模型应用工程」重合度最高；纯推理部署 / AI Infra 深水区不作为第一主线。**完成 Step 0-1-2C 后可开始试投并收集反馈，用面试结果决定 Step 3 先做 RAG 还是补 vLLM。**
...
### 1.9 执行者背景：从 C/C++ 转 Python 的最少背景

面向有 C/C++ 背景的读者，以下是最小过渡知识，不要求精通 Python，够做项目即可。
...
**PyTorch CPU vs CUDA 是不同的 pip package。** 默认 `pip install torch` 可能拉 CUDA 构建。本计划 CPU 路径优先；在纯 CPU 环境安装时注意选 CPU wheel，否则可能报错或行为异常：

```bash
pip install torch --index-url https://download.pytorch.org/whl/cpu
```

**Python async/await vs C++ 协程。** FastAPI 基于 Python 的 `async/await`。Python 事件循环通常在单线程内轮转 async 任务；C++20 协程可配合线程池并行。模型推理多为 CPU/GPU 密集型，在 async 路由里直接调 `model.generate()` 可能长时间占用事件循环线程。Step 2 用 `TextIteratorStreamer` 在后台线程跑推理，是一种常见规避方式。

**C/C++ 与 AI 工程概念的对应。** 不需要从零学起。以下是已有知识与 AI 工程概念的对应关系：
...
**超时跳过规则。** 每个任务标注了预计时间；超时 1.5 倍仍未完成时，写「未完成」并跳过，进入下一任务。小闭环交付优先于单任务打磨。

**调试习惯。** 遇到异常先读 traceback 的最后一行，确认异常类型和消息；栈帧按调用展开，通常最靠下的业务代码帧最接近抛错位置。随后结合局部变量、日志或断点验证，不要仅凭栈顶/栈底猜原因。

这张对照表只提示：C/C++ 经验与 AI 工程概念有可迁移处，细节仍须在 Python API 与实测中验证。各 Step 末尾的「与系统编程经验的对应」会举具体例子。

（读完本章后可用以下速查）

> **术语速查（看到不懂的词先翻这里）**：RAG = 先搜索文档再让 AI 回答 | SSE = 服务端用 `text/event-stream` 推送事件流（Chat Completions 流式常见；不等于「一个字一个事件」） | TTFT = 请求到首个输出 token 的耗时（常含排队+Prefill） | TPOT = \((E2EL-\mathrm{TTFT})/(N-1)\)，首 token 后平均间隔；仅 1 个输出 token 时未定义 | Tokenizer = 把文字转成数字的工具 | PagedAttention = 把显存分页管理（像操作系统虚拟内存） | Continuous Batching = 动态打包多个请求一起推理 | KV Cache = 缓存推理中间结果避免重复计算 | Embedding = 把文字变成向量（一串数字）表示语义 | ChromaDB = 本地向量数据库 | HF = HuggingFace | venv = Python 虚拟环境，隔离项目依赖 | 完整口径见 [ai-engineering.md §3.4](ai-engineering.md)
...
**本计划目标：AI 平台后端 / LLM 服务化 / 大模型应用工程**（岗位多、不写 CUDA、不做算法研究）。推理部署 / ML Platform 相关技能作为加分，不作为第一主线。

> **关键澄清**：这个方向的核心能力是 **「能把开源模型跑起来、包成可观测的 API 服务、容器化部署、写出 CI/CD。整个过程有代码和数据，不是纸上谈兵」**。C/C++ 后端经验可直接延伸到服务化、并发与排障；本计划不覆盖算法研究岗所需的训练与 kernel 开发。
...
| Kubernetes | 4/6 | Step 2D（P1 可选） | Minikube Deployment/Service/ConfigMap；P0 先完成 Docker（2C），勿抢主线 |
...
## 三、Step 0：Ownership Recovery

> **定位**：这一步的目的是确认项目 ownership。如果跳过这一步，后续很可能继续变成 AI 生成脚本、用户负责运行命令，最后面试讲不深。

### 3.1 目标

用约 7 天，把本地已有的 `hf-llm-benchmark` 仓库（若尚无同名仓库，用任意「已能跑通一次 HF 推理」的小项目代替）从「AI 生成脚本集合」变成「可解释、可验证、可最小重写的学习项目」。
...
### 4.2 为什么先做第一个技术 Step（Step 1）

Step 0 已确认 ownership；Step 1 是第一个技术 Step。
...
Step 1 不再从「让 AI 生成完整 benchmark 脚本」开始，而是从「手动理解一个最小推理链路」开始。任何 AI 生成代码都必须经过解释、删减、重写、验证四步。
...
**CPU benchmark 太慢怎么办。** Qwen3-0.6B 在普通笔记本 CPU 上约 2-5 tokens/s，生成 512 token 需要 1-3 分钟。按任务 1.6 的 3 prompt × 3 次 = 9 次，单次约 2 分钟，wall clock 约 20–30 分钟。**扩展完整版时**可增至 10 prompt × 5 次 = 50 次，或挂夜间跑批；规模须与任务表和 README 一致。
...
**与系统编程经验的对应。** Tokenizer 的本质是一次字符串编解码（encode → token ids → decode）。C++ 开发者对字符编码（UTF-8 code point → 字节序列）有对应经验。tokenizer 把「字符→字节」的映射换成「子词→token id」，词表（`tokenizer.json`）即映射表。口述时可强调：核心接近查表操作，与 Unicode 编解码思路相近；中文 token 数偏多通常与词表覆盖度有关。
...
| 组件 | 选择 | 原因 |
|------|------|------|
| Web 框架 | FastAPI | ASGI + `StreamingResponse(media_type="text/event-stream")`（或较新的 `EventSourceResponse`）可返回 SSE |
...
| 2B.3 | TTFT/TPOT | 服务端按模型 token 时间戳：TTFT=请求入点→首 token；TPOT=\((E2EL-\mathrm{TTFT})/(N-1)\)（\(N=1\) 记为未定义）。另记首个有效 SSE `content` 事件作体验对照，勿把 role/元数据包当首 token | 1.5h |
| 2B.4 | `/metrics` | prometheus_client：Counter（请求/错误/output tokens）+ **Histogram**（TTFT、TPOT）；标签带 `model`；吞吐用 `rate(output_tokens)` 推导，勿只暴露瞬时 Gauge | 2h |
...
**验收标准**：
- [ ] 每条日志都有 request_id
- [ ] 能说清 TTFT 与 TPOT 的区别（含 \(N=1\) 边界）、以及和 HTTP P99 的关系
- [ ] `/metrics` 能看到至少 3 个指标，且 TTFT/TPOT 为 Histogram（或 Summary）
- [ ] README 有一段「如何定位慢请求」（对齐 engineering §3.4 拆法）
- [ ] 不把「延迟好看」写成「回答质量好」（质量见 Step 3 / intro §3.6）
- [ ] 口述时能区分：本项目是 HF+FastAPI **学习样本**，不是 vLLM 生产 Serving

#### Step 2C：交付闭环（P0）

**问题单**：如果把这个服务交给别人，能否一键启动、验证、知道出问题去哪看？

> P0 交付到 Docker 即可。K8s（Minikube）在 Step 2D，勿写成 Step 2 必做。镜像内跑的是本项目的 HF+FastAPI 学习服务，不是 vLLM 镜像。
...
- [ ] 容器内 `/health` 正常
- [ ] README 有"快速开始 / 验证 / 常见问题"三段，并写明学习路径边界（非生产 vLLM）
- [ ] 至少 1 个测试能验证服务活着

#### Step 2D：工程加分（P1 可选，三选一）
...
│   ├── service.yaml
│   └── configmap.yaml
├── .github/workflows/             # 【P1 可选，三选一】
│   └── ci.yml
├── tests/
│   ├── test_health.py
│   └── test_chat_completions.py
├── bench.py                       # 【P1 可选，三选一】asyncio + httpx 并发压测
└── results/
...
- [ ] Step 2D（P1 可选，三选一）：K8s / CI / 并发压测完成其一
...
**与系统编程经验的对应。** Python 的 `async/await` 与 C++20 协程语法相近，运行模型不同：Python 事件循环多在单线程内轮转，C++ 协程可配合线程池。据此可理解 FastAPI 中不宜在 async 路由里长时间阻塞 `model.generate()`，以及 `TextIteratorStreamer` 用后台线程的常见做法。asyncio + httpx 压测时分离连接超时与读取超时，与非阻塞 I/O 的 socket timeout 分层思路相近。
...
> 「搭建了端到端 RAG 系统：文档切片 → BGE-small Embedding → ChromaDB 检索 → 本地 Qwen3-0.6B 生成带引用答案，召回率 recall@3 达到 `<实测值>`%。也用 LangChain 重写了核心链路，能说清框架封装了什么、手写版多理解到的细节。」
...
**与系统编程经验的对应。** 向量检索的 ANN 索引与 C++ 中的空间索引（KD-Tree、R-Tree）同属最近邻搜索问题。Embedding 向量是高维空间中的点，ChromaDB 的 HNSW 与 KD-Tree 碰撞检测在数学上相近，只是维度从 3 变为 768。
...
**强化路径（租 GPU 2-4h，约 10-20 元，可选）：** 在 T4 上跑通 vLLM GPU 模式，做 HF vs vLLM 的真实 GPU benchmark 对比。用实验数据验证理论推演；产出一份 GPU 对比报告。
...
> 「学习过 vLLM 架构：阅读官方 Architecture Overview，能讲清 PagedAttention、Continuous Batching 和 KV Cache；跑通 `vllm serve` 的 OpenAI 兼容 API，并完成 KV Cache 手算验证。有余力时可阅读 scheduler/block_manager 入口或做 GPU benchmark（吞吐提升 `<实测值>` 倍等）。」

### 7.9 对标的 JD 技能

vLLM ✅ | PagedAttention ✅ | Continuous Batching ✅ | KV Cache ✅ | SGLang(概念) ✅ | 源码入口（有余力）⚠️ | GPU benchmark（可选）⚠️
...
> 以上两段不需要跑任何代码，纯概念讨论。写在理论推演文档中，面试被问到 MoE 或 PD 分离时可引用。

**与系统编程经验的对应。** PagedAttention 本质是内存管理：KV Cache 按 block 分配，Block Table 做逻辑块→物理块两级映射，与操作系统虚拟内存页表**相似但不相同**（逻辑连续、物理可分散，按需分配与回收）。Preemption 在空闲 block 不足时回收旧请求 block，思路接近 page reclaim。

口述时可联系页表与调度：Block Table 缓解 KV 静态预分配碎片；Continuous Batching 让短请求不必等长请求，吞吐常受调度约束。
...
因为 P0 是 AI 工程落地。vLLM 很有价值，但它容易把注意力拉向源码、GPU、调度、性能优化这些更硬的方向。当前阶段只需要会用 `vllm serve`，能讲 PagedAttention / Continuous Batching / KV Cache 的作用即可。已会 RAG、主缺口是推理机制时，可以把 Step 4 紧挨 Step 2 做，Step 3 后移。

### Q3: 用 0.6B 小模型，HR 会不会觉得浅？

单独看模型参数量会偏浅。本计划用两件事说明能力边界：① Step 1 用 Qwen3 + Llama 两个模型，展示多模型通用能力 ② Step 2 重点展示服务化、流式输出、可观测性与排障闭环。面试时强调工程落地能力，模型大小是实验配置。Step 3 用 0.6B 时，说明验证的是管道与指标，不是生成 SOTA。
...
各 Step 的任务清单、验收标准与停止条件见正文 §三–§七，此处只做跨 Step 进度汇总，避免双份维护。

| Step | 正文 | 优先级 | 完成 |
|------|------|:---:|------|
| Step 0 Ownership Recovery | §三 | P0 | [ ] |
| Step 1 HF LLM 推理基础 | §四 | P0 | [ ] |
| Step 2A–2C 流式服务化 | §五（2A/2B/2C） | P0 | [ ] |
| Step 2D 工程加分 | §五 Step 2D | P1 可选，三选一 | [ ] |
| Step 3 RAG 问答 | §六 | P1 | [ ] |
| Step 4 vLLM 概念 | §七 | P2 | [ ] |
...
| v9.4 | 2026-07-17 | **编辑残留清理**：删 v9/新增等编辑标记；Step 0/1 标题统一；状态改计划稿；任务 1.6 规模对齐；Step 2D/4 验收与可选项统一；附录改为总进度表；humanizer 文风修订 |
