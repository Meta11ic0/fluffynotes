...
> 生成日期：2026-06-17 | 状态：执行中 | 最后更新：2026-07-15（v9 深度调研校准版）
...
### 1.0 v9 核心修正：按 C 型工程师方式交付

2026-06-28 根据最新职业画像复盘后，本计划的定位从「AI Infra 学习路线」修正为「AI 工程落地交付路线」。

- 如果只是让 AI 生成脚本，用户负责运行命令，项目即使完成也很难形成面试可讲能力。
- 用户最新画像是 C 型角色：**给方向后，能在混乱项目里收敛问题、推动交付**。
- 因此本计划从 v9 开始改为：每个阶段必须有「问题单 → 小闭环 → 可讲产物」，再继续扩大功能。
...
4. 正反馈来自「我搞懂了一个机制 / 修掉一个问题 / 得到一个数字」，不是来自文件数量。
...
|------|------|------|------|
| Step 0 | Ownership Recovery | AI 辅助边界、最小重写、手动验证 | 先确认项目是「我能讲清的项目」，不是 AI 生成脚本集合 |
| Step 1 | HF Transformers LLM 推理基础 | 模型加载、tokenizer、生成参数、量化、benchmark | 先理解 LLM 本身如何被调用 |
| Step 2 | LLM 流式服务化（2A-2D） | OpenAI 兼容 API、SSE、TTFT/TPOT、日志、Docker、可观测性 | 把模型包装成可交付服务，并能定位问题 |
...
本计划选择「HF Transformers → LLM 流式服务化 → RAG → vLLM 概念」的原因是：它覆盖最常见 AI 产品链路，同时避开 CUDA/C++/大规模训练这些高成本板块。

**证据来源：**
- HuggingFace Transformers：GitHub 约 17 万 stars（2026.7），PyPI 月下载量过亿，说明其是开源模型加载和推理的主流基础库。
...
8 周内完成一个可讲的 LLM 服务化项目，简历上形成第一条可投递叙事线：**「C/C++ 系统工程背景 → HF Transformers 推理机制理解 → OpenAI 兼容流式服务 → TTFT/TPOT 可观测性 → Docker 交付」**。

v9 不追求一开始铺满 4 个项目，而是先保证一个项目真正可讲、可运行、可排障：
...
|------|:---:|---------|---------|:---:|------|
| Step 0 | P0 | 5-7h | 7 天 | ❌ | 代码 ownership, 独立解释, 最小重写 |
| Step 1 | P0 | 8-12h | 1-2 周 | ❌ | HF Transformers, tokenizer, 生成参数, benchmark |
| Step 2A | P0 | 5-7h | 1 周 | ❌ | FastAPI, SSE streaming, OpenAI compatible API |
| Step 2B | P0 | 4-6h | 1 周 | ❌ | request_id, JSON log, TTFT/TPOT, Prometheus |
| Step 2C | P0 | 3-4h | 3-5 天 | ❌ | Docker, README, curl examples, troubleshooting notes |
| Step 2D | P1 | 8-10h | 1-2 周 | ❌ | K8s, CI/CD, concurrency benchmark |
| Step 3 | P1 | 12-17h | 2-3 周 | ❌ | Embedding, ChromaDB, RAG pipeline, LangChain 对比 |
| Step 4 | P2 | 6-10h（GPU 可选 +4h） | 1-2 周 | ⚠️ CPU 为主，GPU 可选 | vLLM 概念, PagedAttention, Scheduler, KV Cache |
| **合计（P0 到 Step 2C）** | | **25-36h** | **约 6-8 周** | | |
| **合计（含 Step 3）** | | **37-53h** | **约 2.5-3 个月** | | |
...
**前置技能自查。** 开始前确认你能做到以下 4 件事（任何一项不会的先花半天补）：① 在终端里 `cd` 到项目目录并运行 `python xxx.py` ② 创建 venv 并 `pip install` 一个包 ③ 用 `git clone` 拉一个仓库 ④ 读懂 Python 报错的 traceback（从下往上看，最后一行是错误类型，往上是调用链）。

**完美主义执行法则。** 本计划的设计哲学是「小闭环、快速交付、80% 即合格」。每个任务标注了预计时间——如果超时 1.5 倍还没完成，写一句"未完成"标记并跳过。做完永远比做完美重要。面试官更想听你讲一个完成了 80% 但有反思的项目，而不是一个"还在打磨"的完美计划。
...
> **定位**：这一步不是为了增加功能，而是为了确认这个项目到底是不是「你的项目」。如果跳过这一步，后续很可能继续变成 AI 生成脚本、你负责运行命令，最后面试讲不深。

### 3.1 目标

用 7 天把已有 `hf-llm-benchmark` 从「AI 生成的脚本集合」变成「我能解释、能验证、能最小重写的学习项目」。
...
| 0.5 | ownership 笔记 | 写 `notes/ownership.md`，回答 5 个问题：脚本解决什么、核心 API 是什么、最容易错在哪里、如何验证、删掉 AI 后我能重写哪部分 | 1h |
...
| 1.9 | 结果整理 + 提交 | README + `notes/ownership.md` + tokenizer 笔记 + benchmark 结果。README 必须包含「哪些代码是 AI 辅助、哪些部分我已能独立解释」 | 1h |
...
基于 Step 1 的模型和推理代码，用 FastAPI 构建一个可交付的 LLM 流式推理服务。第一阶段不追求完整平台，而是证明三件事：

1. 模型能被包装成 OpenAI 兼容 API。
...
- 这一步最能体现用户的 C 型能力：给定方向后，把混乱链路收敛成可运行、可观测、可交付的服务
...
| 2A.3 | 流式原型 | `/health` + `/generate`，用 SSE 逐 token 输出，`curl` 可看到流式返回 | 2h |
| 2A.4 | OpenAI 兼容端点 | `/v1/chat/completions` 支持 `stream: true`，先不做非流式和复杂错误处理 | 2h |
...
**问题单**：如果用户说「接口很慢」，我怎么判断慢在首 token、后续 token、还是总生成长度？

| # | 任务 | 内容 | 预计 |
|---|------|------|------|
| 2B.1 | request_id | 中间件生成 request_id，写入响应头和日志 | 0.5h |
| 2B.2 | JSON 日志 | 每个请求输出 `{request_id, prompt_tokens, output_tokens, total_time_ms}` | 1h |
| 2B.3 | TTFT/TPOT | 记录首 token 延迟和后续 token 平均间隔 | 1.5h |
| 2B.4 | `/metrics` | 用 prometheus_client 暴露请求数、TTFT、TPOT、tokens/s | 2h |
| 2B.5 | 慢请求案例 | 人为构造长 prompt，对比短 prompt，写 1 段分析 | 1h |

**验收标准**：
- [ ] 每条日志都有 request_id
- [ ] 能说清 TTFT 与 TPOT 的区别
- [ ] `/metrics` 能看到至少 3 个指标
- [ ] README 有一段「如何定位慢请求」说明

#### Step 2C：交付闭环（P0）

**问题单**：如果把这个服务交给别人，他能不能一键启动、验证、知道出问题去哪看？
...
> 「我用 FastAPI 把本地 Qwen 模型封装成 OpenAI 兼容的流式推理服务，实现了 SSE 逐 token 输出、request_id、JSON 日志、TTFT/TPOT 指标和 Docker 交付。这个项目重点不是 Demo，而是能回答‘接口为什么慢、首 token 慢还是后续生成慢、如何观测和排障’。」
...
这是纯 Python 背景的候选人几乎不可能自然建立的连接。你不需要写任何 C++ 代码，但这个视角展示了系统级思维——这正是 AI 工程落地和平台后端岗位看重的素质。
...
> 「我有 N 年的 C/C++ 后端开发经验，擅长在复杂系统里收敛问题并推动交付。近期我完成了一个 LLM 服务化项目：先用 HuggingFace Transformers 跑通本地 Qwen/Llama 推理，理解 tokenizer、chat_template 和 generate 流程；再用 FastAPI 封装成 OpenAI 兼容的流式接口，实现 SSE 逐 token 输出、request_id、JSON 日志、TTFT/TPOT 指标、Prometheus 暴露和 Docker 交付。这个项目重点不是 Demo，而是能解释从请求到 token 输出的链路，并能定位首 token 慢、后续生成慢、模型加载或容器配置等问题。后续我会补 RAG 检索增强和 vLLM 概念作为加分项。」
...
因为你的 P0 不是纯推理优化，而是 AI 工程落地。vLLM 很有价值，但它容易把注意力拉向源码、GPU、调度、性能优化这些更硬的方向。当前阶段只需要会用 `vllm serve`，能讲 PagedAttention / Continuous Batching / KV Cache 的作用即可。

### Q3: 用 0.6B 小模型，HR 会不会觉得浅？

单独看会。所以本计划用两件事来对冲：① Step 1 用两个不同模型（Qwen3 + Llama），展示多模型通用能力 ② Step 2 重点展示服务化、流式输出、可观测性和排障闭环。面试时你展示的是工程落地能力，模型大小只是实验配置。
...
不需要。Step 4 的概念对比已经足够。3/6 JD 提到 SGLang，但你展示的是「了解生态、知道差异」而不是每个都要写代码。注意：国内主流实践是 vLLM+SGLang 协同部署——vLLM 负责服务治理与调度，SGLang 处理特定加速场景（如 JSON Schema 强制输出），两者互补而非替代。面试时能说出这一点即加分。

### Q10: 为什么不学 Ollama？

Ollama ~174k stars，是 stars 最高的本地推理工具，但它主打一键体验、封装层级较高。本计划 Step 4 选择 vLLM 的目的是理解 PagedAttention、Continuous Batching 等底层机制——这是 Ollama 的封装掩盖掉的部分。本地快速实验可以用 Ollama（装好就跑），但学习推理架构要从 vLLM 入手。
