...
**阅读建议：** 第一章建框架（约 30 分钟），第二章看历史（**§2.1–2.3 可整段跳读**，从 §2.4 Transformer 起认真读），第三章五个问题 + §3.6 评估（核心），第四章产业与决策（可当附录）。全文约 1-2 小时。
...
文中关键缩写都会解释；里程碑尽量给公开来源。stars / 薪资等行情数字以 [ai-engineering.md §5](ai-engineering.md) 为权威出处，本文只保留趋势判断。
...
用 ChatGPT 问问题、用 Claude 分析文档。这些产品背后，跑的是同一个东西：**大语言模型（LLM，Large Language Model）**。如果也用 Cursor 写代码，它底层也是同一个原理，只是多了一层工具调用。
...
1. **底层本质**：它是在做概率预测，给定前面的字，猜下一个字最可能是什么。
2. **表层体验**：因为对语言、代码、知识都训练得足够多，感觉它像一个"会交流、会做事的人"。
...
所以用户的感受，"AI 像一个大型的人，给清晰指令后，结合操作库匹配动作"，**在产品体验层面是对的**。但在技术层面，更准确的说法是：
...
下面不是「所有历史事件」的堆砌，只选对今天生态影响最大的转折点。

**跳读建议：** 若目标是工程/Infra，**§2.1–2.3 可整段跳过**，从 §2.4 Transformer 读起；早期史只保留下面三行摘要即可。

| 阶段 | 一句话 |
|------|--------|
| 1956–1985 | AI 成为学科名；感知机等早期尝试，几乎无大众产品 |
| 1986–2011 | 反向传播、CNN/LSTM、CUDA：算法与 GPU 并行算力开始咬合 |
| 2012–2016 | AlexNet / word2vec / seq2seq+Attention / AlphaGo：深度学习进入产业与公众视野 |

需要细节与论文链接时，再翻附录「学科与早期历史」。
...
> 参考索引见文末附录。下面从 **Transformer（2017）** 进入与今天直接相关的主线。
...
**真切看到：只要模型够大，很多任务不一定要专门训练，也能"像会做一样"完成。**
...
RAG（Retrieval-Augmented Generation，检索增强生成）的出现，是因为很快发现大模型的两个问题：
1. 知识停在训练时，不知道最新事实
2. 可能会"幻觉"，编造不存在的信息
...
如果把这 5 个问题真正搞懂，对 AI 的理解就不再停留在"会聊天的黑盒"层面，而是开始进入"理解它为什么能跑起来、为什么难、为什么会有这些岗位"的层面。
...
为什么推理能跑这么快？为什么今天所有大模型都用同一个架构？这要回到 2017 年，看 Transformer 是怎么取代 LSTM 的。
...
RAG 能补充模型参数之外的证据，但不保证模型正确使用这些证据。无论是否使用 RAG，生成阶段还会遇到另一个问题：**显存如何被占用，长对话为什么变慢？** KV Cache 是其中一个重要变量。
...
> 延迟指标（TTFT/TPOT）证明「跑得动、能排障」；本节证明「答得对不对、忠不忠于资料」。二者不要混成一个「好不好」。
...
「上了 RAG 就没有幻觉」是误解——§3.3 已说过；评估要分别测检索与生成。
...
4. 再做 2–3 个**故障注入**：坏切块、top-k 过小、坏 Prompt——看指标是否按预期变差（归因）
...
| **AI 算法工程师** | 训练模型、设计架构、调参，造权重的人 | ❌ 不是本指南关注的方向 |
...
所以今天看到的竞争，不只是模型榜单竞争，而是：模型竞争 × 芯片/算力竞争 × 推理系统竞争 × 产品交互竞争 × 开发生态竞争。
...
换句话说：价值不只在「会问 AI」，而在「知道怎样把 AI 接进真实系统」。
...
1. **读完本文**（至少 §1 + §3）
2. **再读 [ai-engineering.md](ai-engineering.md)**：链路边界 + Prefill/Decode + 工种；§5 数据按需
3. **动手看 [ai-infra-project-plan.md](ai-infra-project-plan.md)**：Step 0→2C 为主；RAG / vLLM 按目标加减

若目标是做产品而不是求职：可以先用商业 API 做出可演示闭环，再回头用 plan 的服务化与观测能力压成本、做私有化。

#### 项目路径选择

两条路径都有效：**Python + HF Transformers**（LLM 应用工程主流）或 **C + ONNX Runtime**（边端/嵌入式）。当前 plan 选 Python。详细说明见 [ai-engineering.md §6](ai-engineering.md)。
...
2. **何时上 GPU/vLLM：** 延迟/成本/并发成为真实瓶颈，或客户要求私有化——之前用 API 即可。  
...
今天的大模型产品，不是「突然发明出来的会说话机器」。它是算法、硬件、数据、训练方法和工程基础设施长期累积的结果；2022 年前后才大规模进入大众视野。GPT、Claude、Cursor、DeepSeek，是这条链路在不同形态上的表现。
