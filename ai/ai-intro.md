...
> **初读提示：** 本节用六步追踪一个真实句子的完整旅程，不讲矩阵运算，只建立直觉。如果只想先建立全局概念，可以跳到 §1.5「核心名词速览」，等读完第三章再回来看这一节。想了解每一层的精确矩阵计算和形状变化，见 [ai-engineering.md §2](ai-engineering.md)。
...
有了位置信息，模型现在知道每个词在句子的哪个位置。以句末的「红烧肉」为例，它可以看到前面的「我」「喜欢」「吃」，其中「吃」与它的关系最直接。因果语言模型不能看到当前位置之后的 token。
...
这个句子经过分词后大约有十几个 token。为了展示逐层编码的过程，追踪**最后一个 token「喜欢」**（或 tokenizer 切出的子词「欢」），因为它在句末，经过因果注意力机制能看到前面所有 token，携带的上下文最完整。
...
32 层全部走完后，每个 token 都得到了一个编码了上下文的最终向量。要预测下一个字，模型取**最后一个 token** 的最终向量（本例是「喜欢」或其子词）。这个向量经最后一轮矩阵乘法投影到词表大小，再经 softmax 变成概率分布：
